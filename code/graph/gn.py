...
def load_graph(f,split_symbol=',',skip_rows=0,is_weight=False,extra_vertics=None):
    g = nx.Graph()
    with open(f) as fo:
    ...
    if extra_vertics:
        g.add_nodes_from(extra_vertics)
...
def try_kclique():
    g = load_graph('/home/jiangln/own_weibo/raw_data/facebook/0.edges',split_symbol=' ',is_weight=False)
    kclique(g)
